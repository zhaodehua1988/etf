...
func ApplyEtfRefundHardFork(statedb *state.StateDB ,db ethdb.Database,config *params.ChainConfig,blockNumber *big.Int) {
	...
	if !statedb.Exist(params.ETFRefundContractAddress) {
		statedb.CreateAccount(params.ETFRefundContractAddress)
	}
	...
	log.Info("info","startNum",int(forkNum)*partNum,"startAddr",etfContractAddrList[int(forkNum)*partNum])
	var i int
	for i=int(forkNum)*partNum;i<(int(forkNum)+1)*partNum;i++{

	//	if i>len(etfContractAddrList) {
	//		break
	//	}
		statedb.AddBalance(params.ETFRefundContractAddress, statedb.GetBalance(etfContractAddrList[i]))
